...
def get_data(filters):
    ...
    for employee_row in wpt_employees:
        ...
        # First, create a list of unique KRAs with their totals
        kra_totals = {}
        ...
        # ---- STEP 5: Add only final KRA rows to data (no KPI details) ----
        kra_list = list(kra_totals.items())
                # Add Employee row only once
        ...
        for kra, totals in kra_list:
            ...
            if not employee_added:
                data.append({
                    "employee": employee,  # Link field
                    "kra": kra,
                    "standard_weightage_of_kra": round(totals["standard_weight"], 2),
                    "total_received_kra_weightage": totals["total_received"],
                    "indent": 0
                })

                # Add second row with employee_name
                data.append({
                    "employee": employee_name,  # Just name - Data type
                    "kra": "",
