...
from hrms.hr.doctype.job_opening.job_opening import JobOpening as WebsiteGenerator
...
class JobOpening(WebsiteGenerator):
	def validate(self):
		...
		self.validate_openings_against_staffingplan_requisition()

	def validate_openings_against_staffingplan_requisition(self):
		if self.is_new():
			existing_plan = frappe.db.exists("Job Opening", {"designation": self.designation, "staffing_plan": self.staffing_plan})
			existing_requisition = frappe.db.exists("Job Opening", {"designation": self.designation, "job_requisition": self.job_requisition})
			if existing_plan:
				frappe.throw(f"Job Opening for {self.designation} against Staffing Plan: <b>{self.staffing_plan}</b> already exists")
			elif existing_requisition:
				frappe.throw(f"Job Opening for {self.designation} against Job Requisition: <b>{self.job_requisition}</b> already exists")
